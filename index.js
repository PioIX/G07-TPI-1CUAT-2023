...
app.get('/agenda2030', async function(req, res){
    if (req.session.user != undefined && req.session.user.includes("Fulanito") == false){
        if (req.session.object[0].avatar==null){
            res.render('agenda-20302', {user: req.session.user, img:"user.png"});
        } else {
            res.render('agenda-20302', {user: req.session.user, img: req.session.object[0].avatar});
        }
    } else {
        res.render('agenda-2030', {user: req.session.user});
    };
})
...
app.get('/stats', async function(req, res){
    let baseDatos = await MySQL.realizarQuery(`Select * From Ranking WHERE user = "${req.session.user}"`);
    let labels = [];
    for (let i = 1; i<parseInt(baseDatos[0].games_played)+1;i++){
        labels.push(i.toString())
    }
    let baseDatos2 = await MySQL.realizarQuery(`Select * From Stats WHERE user = "${req.session.user}" ORDER BY game_played ASC`)
    let respuestas = [];
    for (let i = 0; i<baseDatos2.length;i++){
        respuestas.push(baseDatos2[i].corrects_answers.toString())
    }
    let percent = baseDatos[0].correct_answers*100 / (baseDatos[0].games_played*4);
    percent = percent.toFixed(2);
    
    if (req.session.user != undefined && req.session.user.includes("Fulanito") == false){
        if (isNaN(percent)){
            if (req.session.object[0].avatar==null){
                res.render('stats', {user: req.session.user, img:"user.png", label:labels, datas:respuestas, base:baseDatos[0], percent: 0})
            } else {
                res.render('stats', {user: req.session.user, img: req.session.object[0].avatar, label:labels, datas:respuestas, base:baseDatos[0], percent: 0})
            }
        } else {
            if (req.session.object[0].avatar==null){
                res.render('stats', {user: req.session.user, img:"user.png", label:labels, datas:respuestas, base:baseDatos[0], percent: percent})
            }
            res.render('stats', {user: req.session.user, img: req.session.object[0].avatar, label:labels, datas:respuestas, base:baseDatos[0], percent: percent}) 
        }
    } else {
        if (isNaN(percent)){
            res.render('stats', {user: req.session.user, label:labels, datas:respuestas, base:baseDatos[0], percent: 0})
        } else {
            res.render('stats', {user: req.session.user, label:labels, datas:respuestas, base:baseDatos[0], percent: percent})
        }
    }
})
...
app.put('/questionadd', async function(req, res) {
    await MySQL.realizarQuery(`INSERT INTO Questions (question, answer_1, answer_2, answer_correct, category) VALUES ("${req.body.question}", "${req.body.answer_1}", "${req.body.answer_2}", "${req.body.answer_correct}", ${req.body.category});`);
    res.send({validar: true});
});
...
app.put('/questionmodify', async function(req, res) {
    await MySQL.realizarQuery(`UPDATE Questions SET question = "${req.body.question}", answer_1 = "${req.body.answer_1}", answer_2 = "${req.body.answer_2}", answer_correct = "${req.body.answer_correct}", category = ${req.body.category} WHERE idQuestion = "${req.body.idQuestion}";`)
    res.send({validar: true});
});
...
app.post('/upload', async function (req,res){
    if (req.files){    
        ...
        if (req.session.object[0].avatar==null){
            res.render('profile', {info: req.session.object[0], img: "user.png", user: req.session.user});
        }
        else {
            res.render('profile', {info: req.session.object[0], img:req.session.object[0].avatar, user: req.session.user});
        };
...
app.get('/gotogame', async function(req, res){
    ...
    if (req.session.dataBase.length==0){
        ...
        let last_game = await MySQL.realizarQuery(`Select games_played From Ranking Where user = "${req.session.user}"`)
        await MySQL.realizarQuery(`INSERT INTO Stats VALUES ("${req.session.user}", ${req.session.corrects}, ${req.session.incorrects}, ${last_game[0].games_played});`)
        req.session.score = 0;
        req.session.corrects = 0;
        req.session.incorrects = 0;
        req.session.dataBase = undefined;
        ...
        let percent = (req.session.corrects*100)/(req.session.corrects + req.session.incorrects)
        percent = percent.toFixed(2)
        req.session.dataBase.splice(random,1);
        res.render('game', {question: set.question, 
                            count: req.session.score, 
                            right_answer: set.answer_correct, 
                            answer_1: valores[0], 
                            answer_2: valores[1], 
                            answer_3: valores[2]}); 
    }
...
app.put('/randomQuestion', async function(req,res){
    ...
    res.send({corrects: req.session.corrects, incorrects: req.session.incorrects});
});
